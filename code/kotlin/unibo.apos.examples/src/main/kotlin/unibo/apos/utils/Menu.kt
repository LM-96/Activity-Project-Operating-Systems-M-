...
object disabled
@MenuDsl
data class MenuBuilderIdContinuation(val id: String, val enabled: Boolean)
@MenuDsl
data class MenuBuilderDescriptionContinuation(val entry: MenuEntry)
...
class MenuBuilder(private val title: String) {
    ...
    @MenuDsl
    infix fun onUserSelection(action: (MenuEntry) -> Unit) {
        onEntrySelectedCallbacks.add(action)
    }
}

@MenuDsl
class EnumIdMenuBuilder(private val title: String) {
